...
if(isset($_POST['post_update']))
{
    ...
    $query = "UPDATE posts SET category_id='$category_id',name='$name',description='$description',image='$update_filename',status='$status' WHERE id='$post_id'";
...
if(isset($_POST['post_add']))
{
    ...
    $description = $_POST['description'];
    $image = $_FILES['image']['name'];
    //renaming the image to get easier acces
    $image_extension = pathinfo($image, PATHINFO_EXTENSION);
    $filename = time().'.'.$image_extension;

    $status = $_POST['status'] == true ? '1' : '0';

    $query = "INSERT INTO posts(category_id, name,description, image, status) VALUES 
    ('$category_id','$name','$description','$filename','$status') ";
